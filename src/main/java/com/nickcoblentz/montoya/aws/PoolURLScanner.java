...
public class PoolURLScanner extends MyScanner {

    public static String ISSUE_DETAIL ="<p>The following AWS Cognito Pool URL was accessed:</p><ul><li>%s</li></ul><p>From:</p><ul><li>%s</li></ul>";
    public static String ISSUE_NAME ="AWS Cognito POOL URL Found";
    public PoolURLScanner(MontoyaApi api)
    {
        super(api);
    }
    ...
    public List<AuditIssue> Scan(HttpRequestResponse baseRequestResponse)
    {
        ...
        if(CognitoShared.requestNotNull(baseRequestResponse)) {
            ...
            if (url != null) {
                if (CognitoShared.POOL_URL_PATTERN.matcher(url.getHost()).matches()) {
